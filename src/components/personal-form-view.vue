...
<template>
  ...
  <div class="block rounded-lg shadow-2xl bg-white   ">
    ...
    <div class="p-6">
   <div class=" mb-2" required>Name: {{data.fullName}} </div>
  <div class="mb-2" required>  Email: {{data.email}} </div>
  <div class="mb-2" required> Gender: {{data.gender}}</div>
  <div class="mb-2" required>Zipcode: {{data.Zipcode}}</div>
